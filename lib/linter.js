...
function prettierFormat(text, currentOptions, filename, cwd) {
  ...
  if (mergedOptions.listDifferent) {
    ...
    let report
    if (cwdDeps.usePrettierEslint) {
      report = cwdDeps.prettierEslint({
        fallbackPrettierOptions: mergedOptions,
        filePath: filename,
        text: text,
      })
    } else {
...
function getCwdDeps(cwd, currentOptions) {
  ...
  if (!cwdDeps) {
    ...
    let usePrettierEslint = currentOptions.usePrettierEslint === true
    if (usePrettierEslint) {
      try {
        prettierEslintPath = resolve.sync('prettier-eslint', { basedir: cwd })
      } catch (e) {
        try {
          prettierEslintPath = resolve.sync('prettier-eslint')
        } catch (e) {
    ...
    cwdDeps = prettierMap[cwd] = {
      ...
      usePrettierEslint: usePrettierEslint,
      prettierEslint: require(prettierEslintPath),
